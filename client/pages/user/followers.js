...
const Followers = () => {
  ...
  const getFollowers = async () => {
    try {
      const { data } = await axios.get("/get-followers");
      setPeople(data);
    } catch (err) {
      console.log(err);
    }
  ...
  const handleRemoveFollower = async (user) => {
    try {
      const { data } = await axios.put("/remove-follower", { _id: user._id });
      console.log(data);
      ...
      let followers = people.filter((person) => {
        return person._id !== user._id;
      });
      toast.info(`You removed ${user.username}`);
      setPeople(followers);
    } catch (err) {
